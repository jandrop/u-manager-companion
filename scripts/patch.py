...
def patch_bundle() -> bool:
    ...
    new_generate_network = (
        ...
        "                }\n"
        "                const brPorts = await readdir(`/sys/class/net/${name}/brif`).catch(() => []);\n"
        "                if (brPorts.length > 0) return resolvePciSlot(brPorts[0], depth + 1);\n"
        "                return null;\n"
        "            };\n"
        "            const filtered = interfaces.filter((i) =>\n"
        "                !i.iface.startsWith('veth') && !/^br-[a-f0-9]+$/.test(i.iface) && i.iface !== 'docker0'\n"
        "            );\n"
        "            const pciMap = new Map();\n"
        "            await Promise.all(filtered.map(async ({ iface }) => {\n"
        ...
        "                const t1 = snap1.get(iface.iface);\n"
        "                const t2 = snap2.get(iface.iface);\n"
    ...
    network_polling = (
        ...
        "                }\n"
        "                const isDockerVirtual = (name) => name.startsWith('veth') || /^br-[a-f0-9]+$/.test(name) || name === 'docker0';\n"
        "                const interfaces = [];\n"
        "                for (const [iface, sample] of current.entries()) {\n"
        "                    if (isDockerVirtual(iface)) continue;\n"
...
def main() -> int:
    ...
    if changed_pubsub or changed_bundle:
        restart_api()
